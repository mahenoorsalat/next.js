...
describe('static App Shell prefetch attempt', () => {
  ...
  it('speculative: goes straight to runtime for a segment that already reported insufficiency', async () => {
    ...
    // app/speculative-divergent/page.tsx. The difference: the route is
    // allow-runtime and the link is prefetch={true}, so the page segment is
    // also prefetched during the SPECULATIVE phase — and unlike the
    // divergent-cookies fixture, the runtime prefetch of an allow-runtime
    // segment resolves the cookies() read, so the session content itself
    // arrives in that response.
    //
